...
class AosfatosSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):                
        ...
        if len(response.xpath('//p[contains(text(), "Referências:")]/text()').extract()) <= 1:
            fonts = response.xpath('//p[contains(text(), "Referências:")]/following-sibling::p/a/@href').extract()
        else:
            fonts = response.xpath('//p[contains(text(), "Referências:")]/a/@href').extract()
        # response.xpath('//p[contains(text(), "Referências:")]/a/@href').extract()
        yield {
                  'title': title,
                  'fonts': fonts,
                  'content': content,
